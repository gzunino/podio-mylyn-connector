...
public class PodioTasksSettingsPage extends AbstractRepositorySettingsPage {
	...
	public PodioTasksSettingsPage(TaskRepository taskRepository) {
	...
	public class PodioValidator extends Validator {
		...
		public PodioValidator(TaskRepository taskRepository) {
...
//						TracUiPlugin.ID_PLUGIN, message));
		...
		public void validate(IProgressMonitor monitor) throws MalformedURLException {
			AuthenticationCredentials cred = taskRepository.getCredentials(AuthenticationType.REPOSITORY);
			
			try {
				PodioClient.getClient(cred.getUserName(), cred.getPassword()).connect();
				...
				setStatus(RepositoryStatus.createStatus(repositoryUrl, IStatus.ERROR,
						PodioUIPlugin.PLUGIN_ID,
						"Authentication credentials are not valid : "+ e));
			}
